...
def calculate_t_channel_error(t_tensor, error_tensor):
    ...
    np.save('reproduce/scale{}_eta{}_step{}/imagenet/w{}a{}/linear_correct/idx_bias.npy'.format(scale, ddim_eta, ddim_step, n_bits_w, n_bits_a), bias_arr)
    print(bias_table)
...
def get_kt_dict_imagenet(data_tensor, error_tensor, t_tensor):
    ...
    for k in t_data_dict.keys():
        ...
        print("flatten shape ", flatten_data.shape)
    ...
    print('r_dict: ', r_dict)
    print(kt_dict)
    np.save('reproduce/scale{}_eta{}_step{}/imagenet/w{}a{}/linear_correct/kt.npy'.format(scale, ddim_eta, ddim_step, n_bits_w, n_bits_a), kt_dict, allow_pickle=True)

def get_t_residualerror_std_dict(t_tensor, data_tensor, error_tensor):
    '''
        need kt first for calculating residual error
    '''
    kt_dict = np.load('reproduce/scale{}_eta{}_step{}/imagenet/w{}a{}/linear_correct/kt.npy'.format(scale, ddim_eta, ddim_step, n_bits_w, n_bits_a), allow_pickle=True).item()
    t_std_dict = {}
    for i in range(len(t_tensor)):
        int_t = t_tensor[i].item()
    ...
    print(t_std_dict)
    np.save("reproduce/scale{}_eta{}_step{}/imagenet/w{}a{}/linear_correct/t_std_dict.npy".format(scale, ddim_eta, ddim_step, n_bits_w, n_bits_a), t_std_dict, allow_pickle=True)


if __name__ == '__main__':
    data_error_t_list = torch.load('reproduce/scale{}_eta{}_step{}/imagenet/w{}a{}/data_error_t_w{}a{}_scale{}_eta{}_step{}.pth'.format(scale, ddim_eta, ddim_step, n_bits_w, n_bits_a, n_bits_w, n_bits_a, scale, ddim_eta, ddim_step), map_location='cpu')  ## replace error file here
    data_list = []
    error_list = [] 
    t_list = []

...
def tensor_wise_gaussian_model():
    data_error_t_list = torch.load('reproduce/scale{}_eta{}_step{}/imagenet/w{}a{}/data_error_t_w{}a{}_scale{}_eta{}_step{}.pth'.format(scale, ddim_eta, ddim_steps, n_bits_w, n_bits_a, n_bits_w, n_bits_a, scale, ddim_eta, ddim_steps), map_location='cpu')  ## replace error file here
    data_list = []
    error_list = [] 
    t_list = []
    quant_list = []
    ...
    np.save("reproduce/scale{}_eta{}_step{}/imagenet/w{}a{}/gaussian_correct/mu_dict.npy".format(scale, ddim_eta, ddim_steps, n_bits_w, n_bits_a), mu_dict, allow_pickle=True)
    np.save("reproduce/scale{}_eta{}_step{}/imagenet/w{}a{}/gaussian_correct/cov_dict.npy".format(scale, ddim_eta, ddim_steps, n_bits_w, n_bits_a), cov_dict, allow_pickle=True)
...
def tensor_wise_integeral_gaussian_model():
    ...
    np.save("reproduce/scale{}_eta{}_step{}/imagenet/w{}a{}/gaussian_correct/mu_dict_new.npy".format(scale, ddim_eta, ddim_steps, n_bits_w, n_bits_a), mu_dict, allow_pickle=True)
    np.save("reproduce/scale{}_eta{}_step{}/imagenet/w{}a{}/gaussian_correct/cov_dict_new.npy".format(scale, ddim_eta, ddim_steps, n_bits_w, n_bits_a), cov_dict, allow_pickle=True)
...
def channel_wise_gaussian_model():
    ...
    np.save("reproduce/scale{}_eta{}_step{}/imagenet/w{}a{}/channel_wise_gaussian_correct/mu_dict.npy".format(scale, ddim_eta, ddim_steps, n_bits_w, n_bits_a), mu_dict, allow_pickle=True)
    np.save("reproduce/scale{}_eta{}_step{}/imagenet/w{}a{}/channel_wise_gaussian_correct/cov_dict.npy".format(scale, ddim_eta, ddim_steps, n_bits_w, n_bits_a), cov_dict, allow_pickle=True)

def load_gaussian_params():

    mu = np.load('reproduce/scale3.0_eta0.0_step20/imagenet/w{}a{}/channel_wise_gaussian_correct/mu_dict_4.npy'.format(n_bits_w, n_bits_a), allow_pickle=True).item()
    cov = np.load('reproduce/scale3.0_eta0.0_step20/imagenet/w{}a{}/channel_wise_gaussian_correct/cov_dict_4.npy'.format(n_bits_w, n_bits_a), allow_pickle=True).item()
    x = mu
...
if __name__ == '__main__':
    ...
    # z = np.zeros(2)
    # t = torch.zeros(2).float().cuda()
    # x = torch.tensor(1)
    # x_ = torch.reshape(x, (1,1,1,1))
    # one = torch.ones(1,1,1,1)
    # xx = torch.tensor([[1.0, 0.0],[0.0, 1.0]]).float().cuda()
    # mu = np.load("correct_data/gaussians_250steps_w4a8/mu_dict.npy", allow_pickle=True).item()
    # cov = np.load("correct_data/gaussians_250steps_w4a8/cov_dict.npy", allow_pickle=True).item()
    # x = mu

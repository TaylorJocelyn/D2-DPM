...
def calculate_t_channel_error(t_tensor, error_tensor):
    ...
    np.save('reproduce/lsun_{}_eta{}_step{}/w{}a{}/linear_correct/idx_bias.npy'.format(data_type, ddim_eta, ddim_step, n_bits_w, n_bits_a), bias_arr)
    print(bias_table)
...
def get_kt_dict_imagenet(data_tensor, error_tensor, t_tensor):
    ...
    print(kt_dict)
    np.save('reproduce/lsun_{}_eta{}_step{}/w{}a{}/linear_correct/kt.npy'.format(data_type, ddim_eta, ddim_step, n_bits_w, n_bits_a), kt_dict, allow_pickle=True)

def get_t_residualerror_std_dict(t_tensor, data_tensor, error_tensor):
    '''
        need kt first for calculating residual error
    '''
    kt_dict = np.load('reproduce/lsun_{}_eta{}_step{}/w{}a{}/linear_correct/kt.npy'.format(data_type, ddim_eta, ddim_step, n_bits_w, n_bits_a), allow_pickle=True).item()
    t_std_dict = {}
    for i in range(len(t_tensor)):
        int_t = t_tensor[i].item()
    ...
    print(t_std_dict)
    np.save("reproduce/lsun_{}_eta{}_step{}/w{}a{}/linear_correct/t_std_dict.npy".format(data_type, ddim_eta, ddim_step, n_bits_w, n_bits_a), t_std_dict, allow_pickle=True)


if __name__ == '__main__':
    data_error_t_list = torch.load('reproduce/lsun_{}_eta{}_step{}/w{}a{}/data/data_error_t_w{}a{}_eta{}_step{}.pth'.format(data_type, ddim_eta, ddim_step, n_bits_w, n_bits_a, n_bits_w, n_bits_a, ddim_eta, ddim_step), map_location='cpu')  ## replace error file here
    data_list = []
    error_list = [] 
    t_list = []

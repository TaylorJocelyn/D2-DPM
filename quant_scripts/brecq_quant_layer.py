...
class QuantModule(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        ops = 0
        if len(self.org_weight.shape) == 2: # QLinear
            output_size, input_size = self.org_weight.shape
            # ops = 2 * output_size * input_size * 8 * 8
            ops = 2 * output_size * input_size * 32 * 32
        else: # QConv
            out_chn, in_chn, k_h, k_w = self.org_weight.shape
            _, _, out_h, out_w = out.shape
            # ops = 2 * out_h * out_w * out_chn * in_chn * k_h * k_w * 8 * 8
            ops = 2 * out_h * out_w * (in_chn * k_h * k_w * out_chn) * 32 * 32
        gb.add_bops(ops)

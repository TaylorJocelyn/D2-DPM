...
from quant_scripts.brecq_uncond.brecq_data_utils_uncond import save_grad_data, save_inp_oup_data
...
def layer_reconstruction(model: QuantModel, layer: QuantModule, cali_images:torch.Tensor, cali_t:torch.Tensor,
    ...
    cached_inps, cached_outs = save_inp_oup_data(model, layer, cali_images, cali_t, asym, act_quant, batch_size)
    # print('cached_inps shape: ', cached_inps.shape)
    # print('cached_outs shape: ', cached_outs.shape)
    
    if opt_mode != 'mse':
        cached_grads = save_grad_data(model, layer, cali_images, cali_t, act_quant, batch_size=batch_size)
    else:
        cached_grads = None
    device = 'cuda'
    for i in range(iters):
        idx = torch.randperm(cached_inps.shape[0])[:batch_size]

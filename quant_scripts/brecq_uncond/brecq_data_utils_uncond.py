...
from quant_scripts.brecq_quant_layer import QuantModule, Union
from quant_scripts.brecq_uncond.brecq_quant_model_uncond import QuantModel


def save_inp_oup_data(model: QuantModel, layer, cali_images, cali_t,
    ...
    for i in range(int(cali_images.size(0) / batch_size)):
        cur_inp, cur_out = get_inp_out(cali_images[i * batch_size:(i + 1) * batch_size],cali_t[i * batch_size:(i + 1) * batch_size])
...
def save_inp_oup_data_block(model: QuantModel, layer: QuantModule, cali_images, cali_t,
                      asym: bool = False, act_quant: bool = False, batch_size: int = 32, keep_gpu: bool = True):
    """
    Save input data and output data of a particular layer/block over calibration dataset.
    ...
    for i in range(int(cali_images.size(0) / batch_size)):
        cur_inp0, cur_inp1, cur_out = get_inp_out(cali_images[i * batch_size:(i + 1) * batch_size],cali_t[i * batch_size:(i + 1) * batch_size])
        cached_batches.append((cur_inp0.cpu(), cur_inp1.cpu(), cur_out.cpu()))

    cached_inps0 = torch.cat([x[0] for x in cached_batches])
    cached_inps1 = torch.cat([x[1] for x in cached_batches])
    cached_outs = torch.cat([x[2] for x in cached_batches])
    torch.cuda.empty_cache()
    if keep_gpu:
        cached_inps0 = cached_inps0.to(device)
        cached_inps1 = cached_inps1.to(device)
        cached_outs = cached_outs.to(device)
    return cached_inps0, cached_inps1, cached_outs


def save_grad_data(model: QuantModel, layer: QuantModule, cali_images, cali_t,
    ...
    for i in range(int(cali_images.size(0) / batch_size)):
        cur_grad = get_grad(cali_images[i * batch_size:(i + 1) * batch_size],cali_t[i * batch_size:(i + 1) * batch_size])
...
class GetLayerInpOut:
    def __init__(self, model: QuantModel, layer: QuantModule,
    ...
    def __call__(self, cali_images, cali_t):
        self.model.eval()
        ...
        with torch.no_grad():
            try:
                _ = self.model(cali_images.to(self.device),cali_t.to(self.device))
        ...
        for m in self.layer.modules():
            if isinstance(m, QuantModule):
                m.set_quant_state(True, self.act_quant)
        self.model.train()
...
def quantize_model_till(model: QuantModule, layer: QuantModule, act_quant: bool = False):
    """

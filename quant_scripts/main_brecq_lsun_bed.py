...
sys.path.append('.')
from ldm.models.diffusion.ddim import DDIMSampler, DDIMSampler_gaussian_quantCorrection_imagenet, DDIMSampler_channel_wise_implicit_gaussian_quantCorrection_imagenet
...
def convsample_ddim(model, steps, shape, eta=1.0
    ...
    ddim = DDIMSampler_gaussian_quantCorrection_imagenet(model, num_bit=4, correct=True)
    bs = shape[0]
    shape = shape[1:]
...
if __name__ == "__main__":
    ...
    logdir = 'reproduce/lsun_bedroom_eta1.0_step200/w4a8/generated_samples'
    ...
    if model.cond_stage_model is None:
        ...
        grid = make_grid(grid, nrow=n_samples // batch_size)

        # to image
        grid = rearrange(grid, 'c h w -> h w c').cpu().numpy()
        image_to_save = Image.fromarray(grid.astype(np.uint8))
        image_to_save.save("reproduce/lsun_bedroom_eta1.0_step200/w4a8/generated_samplesimagenet_brecq_w{}a{}_step{}_eta{}_scale{}_corrected.jpg".format(n_bits_w, n_bits_a, ddim_steps, ddim_eta, scale))

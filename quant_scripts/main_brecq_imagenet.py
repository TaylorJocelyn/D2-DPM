...
from ldm.models.diffusion.ddim import DDIMSampler, DDIMSampler_channel_wise_implicit_gaussian_quantCorrection_imagenet, DDIMSampler_implicit_gaussian_quantCorrection_imagenet, DDIMSampler_improved_gaussian_quantCorrection_imagenet, DDIMSampler_integral_gaussian_quantCorrection_imagenet, DDIMSampler_quantCorrection_imagenet, DDIMSampler_gaussian_quantCorrection_imagenet, DDIMSampler_channel_wise_explicit_gaussian_quantCorrection_imagenet
...
if __name__ == '__main__':
    ...
    cali_images, cali_t, cali_y = get_calibration_set('reproduce/scale1.5_eta1.0_step250/imagenet/w4a8/imagenet_input_1000classes.pth')
#    cali_images, cali_t, cali_y = get_train_samples(data_loader, num_samples=1024)
    ...
    ckpt = torch.load('/home/zq/PTQD/reproduce/scale1.5_eta1.0_step250/imagenet/w4a8/weights/quantw{}a{}_ldm_brecq_1000classes.pth'.format(n_bits_w, n_bits_a), map_location='cpu')
    ...
    # macs = torchprofile.profile_macs(model.model.diffusion_model, (xt, t, contex))
    # print(f"FLOPs: {macs * 2}")
    # params = torchprofile.profile_params(model)

    # sampler = DDIMSampler(model)
    sampler = DDIMSampler_channel_wise_implicit_gaussian_quantCorrection_imagenet(model, num_bit=4, correct=True)
    # sampler = DDIMSampler_quantCorrection_imagenet(model, num_bit=4, correct=True)
    # sampler = DDIMSampler_channel_wise_explicit_gaussian_quantCorrection_imagenet(model, num_bit=4, correct=True)
    ...
    all_samples = list()
    import ldm.globalvar as gb
    cali_bops = gb.get_bops()
    print("cali_bops: ", cali_bops)
    ...
    # tot_bops = gb.get_bops()
    # print("total bops: ", tot_bops)
    # print("1 iter bops: ", tot_bops - cali_bops)
    ...
    image_to_save.save("reproduce/scale{}_eta{}_step{}/imagenet/w{}a{}/save_data/cwi_correct_{}_step250.png".format(scale, ddim_eta, ddim_steps, n_bits_w, n_bits_a, seed))
    torch.save(all_samples, "reproduce/scale{}_eta{}_step{}/imagenet/w{}a{}/save_data/cwi_correct_{}_step250.pth".format(scale, ddim_eta, ddim_steps, n_bits_w, n_bits_a, seed))
